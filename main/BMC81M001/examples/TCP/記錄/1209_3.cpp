  ...
  // =========================
    ...
    if (idx >= 2 &&
      ...
      if (isBodyMovementPacket(buf, idx)) {
        ...
        // ======= 累積到伺服器指定秒數 → 播聲 =======
        if (serverTargetSeconds > 0 && quietSeconds >= serverTargetSeconds) {
          myBMV31T001.playVoice(voice_table[0]);     // 播放音檔
          Wifi.writeDataTcp(1, "8");
          quietSeconds = 0;// 避免連續觸發

...
bool fallDetected = false;//是否跌倒
bool hasPerson = false;   // 是否有人：Human existence packet
// 暫存
...
void processIncomingByte(byte data) {
  ...
  if (!inPacket) {
    ...
    // 檢查是否為 Human existence 封包（有人 / 無人）
    if (isHumanExistPacket(buf, idx)) {
       uint8_t existValue = buf[6];
       if (existValue == 0x00) {
            hasPerson = true;
           Serial.println("有人 → 開始計時");
       } else {
            hasPerson = false;
           Serial.println("沒人 → 暫停計時");
       }
    }
    // 檢查是否為 Body movement 封包（活動量）
...
//有無人封包
bool isHumanExistPacket(const byte *buf, int len) {
    if (len < 10) return false;

    // 53 59 80 01 00 01 XX chk 54 43
    if (buf[0] == 0x53 &&
        buf[1] == 0x59 &&
        buf[2] == 0x80 &&
        buf[3] == 0x01 &&

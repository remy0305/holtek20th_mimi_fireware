...
// 設定靈敏度（最高3）的命令
//const byte sensitivityCmd[] = { 0x53, 0x59, 0x83, 0x0D, 0x00, 0x01, 0x01, 0x40, 0x54, 0x43 };//靈敏度1
const byte sensitivityCmd[] = { 0x53, 0x59, 0x83, 0x0D, 0x00, 0x01, 0x02, 0x40, 0x54, 0x43 };//靈敏度2
...
// ===== 活動量計時 =====
int quietSeconds = 0;          // 累積靜止秒數
int serverTargetSeconds = -1;  // 伺服器指定的秒數
//=====12/15新增
bool presenceActive = false;   // 目前是否有人
unsigned long presenceStartMs = 0;
unsigned long presenceAccumMs = 0; // 累積時間（可暫停）
...
void processIncomingByte(byte data) {
  ...
  // ===== 封包中 =====
    ...
    if (idx >= 2 &&
      ...
      printPacket(buf, idx);
      // 0. 人體存在封包（只控制 presenceActive）
      bool occupied;
      if (isPresencePacket(buf, idx, occupied)) {
        presenceActive = occupied;
        if (occupied) {
          Serial.println("Presence: OCCUPIED");
        } else {
          Serial.println("Presence: UNOCCUPIED");
        }
      }
      // 1. 跌倒封包
      ...
      // 2. Body Movement（只在「有人」時才計時）
      if (isBodyMovementPacket(buf, idx)) {

        int movementValue = buf[6];  // 0~100

        if (presenceActive) {
          if (movementValue < 10) {
            quietSeconds++;          // 累積靜止
          } else {
            quietSeconds = 0;        // 有動就歸零
          }
        }
        // 沒人：完全不動 quietSeconds（暫停）
        Serial.print("Quiet Seconds = ");
        Serial.println(quietSeconds);

        if (serverTargetSeconds > 0 &&
            quietSeconds >= serverTargetSeconds) {
...
//=======判斷是否又收到無有人的封包
bool isPresencePacket(const byte *buf, int len, bool &occupied) {
  if (len != 10) return false;
  if (buf[0] != 0x53 || buf[1] != 0x59) return false;

  // Ctrl = 0x80（人體存在）, Cmd = 0x01（存在狀態）
  if (buf[2] != 0x80 || buf[3] != 0x01) return false;

  if (buf[4] != 0x00 || buf[5] != 0x01) return false;
  if (buf[8] != 0x54 || buf[9] != 0x43) return false;

  // buf[6] = 狀態
  occupied = (buf[6] == 0x00); // 0x00 = 有人
  return true;
}
// ========== 功能：發送封包 + 接收回應 =====================
void sendAndReceive(const byte *cmd, int len, const char *desc) {

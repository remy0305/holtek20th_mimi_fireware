...
void loop() {
  ...
  if (recv.length() > 0) {
    ...
    if (value >= 0) {
      ...
      historyIndex = 0;
      historyCount = 0;
      Serial.println("History cleared, new target set.");
    }
  } 
    ...
    if (idx >= 2 &&
      ...
      if (isBodyMovementPacket(buf, idx)) {
        ...
        if (serverTargetSeconds > 0 && historyCount >= serverTargetSeconds) {
          ...
          Serial.print("Consecutive Low Count = ");
...
int parseServerNumber(const String &msg) {
  ...
  int start = msg.indexOf('[');   // 找到左中括號的位置
  int end   = msg.indexOf(']');   // 找到右中括號的位置

  // 如果找不到中括號或位置反了，就表示格式錯誤

...
bool isBodyMovementPacket(const byte *buf, int len) {
  ...
  // 判斷封包開頭是否是 Body movement packet
  if (buf[0] == HEADER1 && buf[1] == HEADER2 && buf[2] == 0x80 && buf[3] == 0x03 &&
      buf[4] == 0x00 && buf[5] == 0x01 && buf[len - 2] == TAIL1 && buf[len - 1] == TAIL2) {
    return true;
  }
  return false;
}
